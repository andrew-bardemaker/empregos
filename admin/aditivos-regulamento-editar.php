...
$tpl->assignInclude('content', './tpl/aditivos-regulamento-editar.html');
$tpl->prepare();
...
$sql = "SELECT * FROM bejobs_aditivos_regulamento WHERE id = $id"; //or die();
//print_r($sql);
$query = $dba->query($sql);
$qntd = $dba->rows($query);
if ($qntd > 0) {
	...
	header('Location: aditivos-regulamento');
}
