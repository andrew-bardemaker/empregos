...
$tpl->assignInclude('content', './tpl/compras-editar.html');
$tpl->prepare();
...
if ($qntd > 0) {
	...
	$tpl->assign('data_emissao', datetime_date_ptbr($vet['data_emissao']));	
	$tpl->assign('hora_emissao', datetime_time_full_ptbr($vet['data_emissao']));
	...
	$sql2   = "SELECT p.codigo, p.titulo, cp.qtde, cp.vl_unit
			   FROM bejobs_compras_produtos AS cp
			   INNER JOIN bejobs_produtos AS p
			   WHERE cp.nfce_chave_acesso = '$chave_acesso'
			   AND cp.id_produto = p.codigo";
	...
	if ($qntd2 > 0) {
		for ($i=0; $i<$qntd2; $i++) {
			...
			$tpl->assign('id_produto', $vet2[0]);
			$tpl->assign('titulo', $vet2[1]);
			$tpl->assign('qtde', intval($vet2[2]));
			$tpl->assign('vl_unit', moeda($vet2[3]));
