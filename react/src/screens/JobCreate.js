...
import { Box, Button, Card, CardMedia, Checkbox, CircularProgress, createTheme, FormControl, FormHelperText, Grid, InputLabel, MenuItem, Select, TextField, Typography } from '@mui/material';
    ...
    return (
        <Box sx={styles.centralize}>
            <Grid component={Card} container spacing={1} sx={styles.userProfileBox}>
                ...
                <Grid item xs={6} sm={3}>
                    <FormControl>
                        <Select
                            ...
                            fullWidth
                        >
                            <MenuItem value={''}>- Selecione a forma de contratação -</MenuItem>
                            {
                ...
                <Grid item xs={6} sm={2}>
                    <TextField disabled={city === 0} fullWidth label="Bairro" value={location} onChange={(e) => setLocation(e.target.value)} />
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                    ...
                    <Button disabled={isLoading} startIcon={<Work />} color="primary" variant="contained" fullWidth sx={{ mt: '1.5em' }} onClick={() => createJob()}>
                        {isLoading ? <CircularProgress /> : "Criar vaga"}
                    </Button>
                    {message && <Typography color={'green'} mt="1em">{message}</Typography>}
                    {errorMessage && <Typography color={'red'} mt="1em">{errorMessage}</Typography>}
                    {newJob && <Button startIcon={<RemoveRedEye />} variant='contained' component={Link} to={`/MinhasVagasDetalhes/${newJob}`} mt="1em">Visualizar oportunidade criada</Button>}
                </Grid>
            </Grid>
        </Box>
    )
